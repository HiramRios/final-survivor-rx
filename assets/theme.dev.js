...
(function (scrollLock, themeAddresses, themeCurrency, Rellax, Flickity, FlickityFade, themeImages) {
    ...
    */
    ...
    class CartItems extends HTMLElement {
      ...
      async getCart() {
        const getCartApi = await fetch('/cart?view=metafields');
        const getCartRes = await getCartApi.json();
        
        if (document.querySelector('.fee-product-handle')) {
          const feeProductId = document.querySelector('.fee-product-handle').dataset.id;
        
          if (feeProductId) {
            const hasPrimary = getCartRes.items.some(item => item.is_primary === true);
            const giftProductId = feeProductId;
            
            if (hasPrimary) {
              const giftExists = getCartRes.items.some(item => item.id == giftProductId);
              if (!giftExists) {
              ...
              // No primary product → Remove gift product if it exists
              const giftItem = getCartRes.items.find(item => item.id == giftProductId);
        ...
        async function reorderCart() {
          // Fetch the current cart contents using the Shopify AJAX API
          ...
          cart.items.forEach(item => {
            ...
            else if (item.properties._gift) {
              giftProducts.push(item);
            } 
            else if (!item.properties._okc_quantity && !item.properties._okc_toa) {
          ...
          // Reorder items: primary products first, gift products second, addon products last
          const reorderedItems = [...addonProducts, ...giftProducts, ...primaryProducts];
    
          console.log(reorderedItems, 'reorder');

          
          
          await clearCart();
          
          await addToCartBulk(reorderedItems);
        }
        ...
        async function addToCartBulk(items) {
          let data = {
            items: items.map(item => {
              let sellingPlan = item.selling_plan_allocation ? item.selling_plan_allocation.selling_plan : null;
              return {
                id: item.variant_id,
                quantity: item.quantity,
                properties: item.properties || {},
                ...(sellingPlan && { selling_plan: sellingPlan.id }) // Add selling plan only if it exists
              };
            })
          };
          ...
          console.log('Items added to cart:', responseData);
        }
        ...
        fetch(theme.routes.cart_url + "?section_id=api-cart-items")
          ...
          .then((t) => {
            ...
            e.innerHTML = t;
            const s = e.querySelector(selectors$U.apiContent);
            this.build(s);

            if (document.querySelector('.max_limit_issue')) {
              document.querySelector('.max_limit_issue').remove();
            }
    ...
    */
    ...
    class CollapsibleElements extends HTMLElement {
      ...
      open(collapsible) {
        ...
        if (!collapsible.closest('collection-filters-form')) {
          setTimeout(() => {
    ...
    class ProductForm extends HTMLElement {
      ...
      updateAddToCartState(formState) {
        ...
        addToCartText?.forEach((element) => {
          ...
          const isPack = element.dataset.isPack;
          const isGift = element.dataset.gift;
          ...
          if(isGift == 'true') {
            ...
            if (isPack == 'true') {
              element.textContent = btnText.replace('cart', 'pack') + ' - ' + price;
            } else {
              element.textContent = btnText + ' - ' + price;
            }
          }
        });
